...
class StreamlitFunction:
    def __init__(self, workdir):
        self.workdir = workdir
        self.df_data, self.df_group = self.generate_default_data(workdir)
        if self.df_data is None or self.df_group is None:
            return  # 数据文件不存在时，不继续执行
        
    # 生成默认数据集函数
    def generate_default_data(self, workdir):
        """
        生成默认数据集
        """
    ...
    def run(self):
        ...
        with left_column:
            self.handle_left_column()
        with right_column:
            self.handle_right_column()

    def handle_left_column(self):
        self.checkbox_data = st.checkbox('是否要导入自己数据？')
        uploaded_data = None
        if self.checkbox_data:
            self.uploaded_data = st.file_uploader("上传你的数据文件（注意分隔符号）。", type=['csv', 'tsv', 'txt', 'xlsx', 'xls'])
    ...
    # 设置特殊参数
        ...
        if_yaxis = st.radio('Y 轴图', ('yes', 'no'), horizontal=True, index=0)
        if_points = st.radio('是否显示数据点？', ('all', 'outliers', 'suspectedoutliers', 'False'), horizontal=True, index=3)
    ...
    # 通用参数设置
        ...
        legent_text = st.text_input('图例标题（不显示则为空）:')
        ...
        # 透明度
        opacity = float(st.slider('透明度:', min_value=0.0, max_value=1.0, value=1.0, step=0.1))
        ...
        #     error_placeholder.error(f'参数设置时出错：{e}')

    def handle_right_column(self):
        # try:
        # 在页面顶部创建一个占位符
        error_placeholder = st.empty()
        if self.checkbox_data and self.uploaded_data is not None:
        ...
        # 根据用户的选择调整数据读取方式
            if user_first_column and user_first_index:
                # 首行为列名，首列为索引
                df_data = load_and_read_data(self.uploaded_data, index_col=0, header=0)
            elif user_first_column:
                # 仅首行为列名
                df_data = load_and_read_data(self.uploaded_data, index_col=None, header=0)
            elif user_first_index:
                # 仅首列为索引
                df_data = load_and_read_data(self.uploaded_data, index_col=0, header=None)
            else:
                # 首行首列均不作为名称
                df_data = load_and_read_data(self.uploaded_data, index_col=None, header=None)
            ...
            st.subheader('原始数据')
            st.dataframe(self.df_data)  # 使用 st.dataframe 显示数据
...
# 绘图
        ...
        # except Exception as e:
        #     error_placeholder.error(f'绘图时出错：{e}')

def Box_Plot(self, df_data, df_group=None, if_selected_color='离散颜色集', group_names=None, 
...
def main(workdir):
    function = StreamlitFunction(workdir)
    function.run()
