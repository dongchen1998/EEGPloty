...
def layout_interface(workdir=None):
    ...
    # 在左边的列中放置复选框和文件上传器：导入数据集
        ...
        # 设置特殊参数
            ...
            if_box = st.radio('是否显示箱线图？', ('yes', 'no'), horizontal=True, index=0)
            if_points = st.radio('是否显示数据点？', ('False', 'all', 'outliers', 'suspectedoutliers'), horizontal=True, index=0)
            if_side = st.radio('小提琴显示区域', ('both', 'positive', 'negative'), horizontal=True, index=0)
    ...
    # 在右边的列中放置表格视图
        with right_column:
            ...
            # if checkbox_group and uploaded_group is not None:
            #     df_group = load_and_read_data(uploaded_group)
            #     # 询问用户首行和首列是否为名称
            #     user_first_row = st.checkbox('首行为名称？')
            #     user_first_column = st.checkbox('首列为名称？')
            #     # 如果用户选择首行和首列为名称，则将首行和首列作为索引
            #     if user_first_row:
            #         df_group = df_group.set_index(df_group.columns[0])
            #     if user_first_column:
            #         df_group = df_group.set_index(df_group.index[0])
            #     # 如果DataFrame不是None，显示数据表
            #     if df_group is not None:
            #         st.subheader('分组信息：')
            #         st.dataframe(df_group)  # 使用st.dataframe显示数据
            #         # group_rep = AgGrid(df_group, editable=True, fit_columns_on_grid_load=True)
            #         # df_new_group = group_rep['data']
            # else:
            #     st.subheader('分组信息')
    ...
    # 绘图
            ...
            # st.subheader('Result：')
                violin_plot = Violin_Plot(df_data, 
                                        df_group=df_group,
                ...
                st.plotly_chart(violin_plot, use_container_width=False, theme=None)

    except Exception as e:
        error_placeholder.error(f'参数设置错误，请检查：{e}')

# 绘图函数
def Violin_Plot(df_data, df_group=None, if_selected_color='离散颜色集', group_names=None, 
                group_color=None, if_lg='no', if_yaxis='yes', if_box='yes', if_points='no', 
                if_side='both', if_no_bg='no',
    ...
    if_box = True if if_box == 'yes' else False
    pointpos = None
    if if_points =='False':
        if_points = False
    elif if_points == 'all':
    ...
    for i, column in enumerate(y_columns):
        ...
        fig.add_trace(go.Violin(x=y if axis == 'x' else None,
    ...
    fig.update_layout(
                      ...
                      plot_bgcolor=plot_bgcolor,  # 设置绘图背景颜色为白色
                      paper_bgcolor=paper_bgcolor,  # 设置整个图表背景颜色为白色
