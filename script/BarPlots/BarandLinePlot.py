...
def generate_default_data(workdir):
    ...
    生成默认数据集
    """
    data_path = os.path.join(workdir, 'input_files', 'Pro_demo_6S_2col_Bar.csv')
    group_path = os.path.join(workdir, 'input_files', 'Pro_demo_6S_2col_Bar_Group.csv')
...
def layout_interface(workdir=None):
    ...
    # 在左边的列中放置复选框和文件上传器：导入数据集
        ...
        # 设置特殊参数
            ...
            if_text = st.radio('是否显示数量', ('yes', 'no'), horizontal=True, index=0)
...
def Bar_Plot(df_data, df_group=None, if_selected_color='离散颜色集', group_names=None, 
             group_color=None, if_text='yes', if_no_bg='no',
    ...
    # 初始化变量
    x_columns = df_data.index.tolist()
    group_names = group_names or x_columns
    ...
    for i, column in enumerate(x_columns):
        y = df_data.iloc[:, 0][column]
        fig.add_trace(go.Bar(
            x=[group_names[i]],  # 对于柱状图，x 是类别名称
            y=[y],  # 这里我们使用 y 的均值
            name=group_names[i],
            marker_color=colors[i % len(colors)],  # 颜色设置
            opacity=opacity,
            text=[y] if if_text == 'yes' else None,
            showlegend=if_legend,
        ))

    fig.add_trace(go.Scatter(
        x=group_names,  # 对于柱状图，x 是类别名称
        y=df_data.iloc[:, 1],  # 这里我们使用 y 的均值
        name=df_data.iloc[:, 1].name,
        marker_color='#404040',  # 颜色设置
        marker_symbol='circle',
        line=dict(color='#404040', width=3),
        opacity=opacity,
        text=df_data.iloc[:, 1] if if_text == 'yes' else None,
        showlegend=if_legend,
        mode='markers+lines',
        yaxis='y2'
    ))
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title='R2', titlefont=dict(size=y_title_font_size), overlaying='y', side='right'),
        legend=dict(x=1.15, title=legent_text, orientation=legend_orientation, font=dict(size=legend_font_size)),
