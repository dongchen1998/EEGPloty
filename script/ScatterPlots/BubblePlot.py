...
def generate_default_data(workdir):
    ...
    生成默认数据集
    """
    data_path = os.path.join(workdir, 'input_files', 'demo_Bubble_enrich_go.tsv')
    ...
    df_data = pd.read_csv(data_path, sep='\t', index_col=False, header=0)
...
def layout_interface(workdir=None):
    ...
    # 设置特殊参数
        ...
        if toggle_state == '离散颜色集':
            ...
            # 创建一个下拉菜单让用户选择特定的颜色
            selected_color = st.multiselect('选择具体的颜色 (至少选择两组)：', selected_colors)  # 返回一个颜色列表
            ...
            # 返回一个颜色列表
            selected_color = st.text_input('至少填入两组（用英文逗号隔开）：', '#FA7F6F, #82B0D2, #8ECFC9, #FFBE7A, #BEB8DC, #E7DAD2, #A1A9D0, #B883D4, #9E9E9E, #CFEAF1')
        ...
        marker_size = st.slider('点大小：', min_value=0, max_value=100, value=30) 
        symbol = st.selectbox('选择点的形状：', symbol_list)
        # sub_col1, sub_col2= st.columns(2) 
        # with sub_col1:
    ...
    # 通用参数设置
        ...
        with sub_col2:
            ...
            # 宽度设置，使用滑块选择
            width = float(st.slider('图片宽度:', min_value=0.0, max_value=30.0, value=9.0, step=0.1))
            # 高度设置，使用滑块选择
            height = float(st.slider('图片高度:', min_value=0.0, max_value=30.0, value=6.0, step=0.1))
            # 透明度
...
def Scatter_Plot(df_data, df_group=None, if_selected_color='离散颜色集', group_names=None, selected_text=None,
    ...
    if if_selected_color == '离散颜色集':
        colors = group_color or getattr(px.colors.qualitative, 'Plotly')
    ...
    #         x=df_data['GeneRatio'],  # 对于柱状图，x 是类别名称
    #         y=df_data['Description'],  
    #         # name=group_names[i],
    ...
    # 数据处理
    df_data.columns = ["Class", "ID", "Description", "Count", "GeneRatio", "P.adjust"]
    df_data["GeneRatio"] = df_data["GeneRatio"].apply(lambda x: round(eval(x), 3)) # GeneRatio列输出处理为浮点数
    df_data['P.adjust'] = df_data['P.adjust'].apply(lambda x: round(x, 6)) # 控制P.adjust列的小数位数
    df_data = df_data.sort_values(by='Count', ascending=False) # 按照Count列降序排列
    df_data = df_data[df_data["P.adjust"] < 0.5] # 过滤P.adjust列小于p_value的行
    df_data = df_data.iloc[:30] # 取前bubble_num行
    fig = px.scatter(
                    df_data,
                    x="GeneRatio",
                    y="Description",
    ...
    fig.update_layout(
        ...
        yaxis=dict(title=y_title, titlefont=dict(size=y_title_font_size), autorange="reversed"),
