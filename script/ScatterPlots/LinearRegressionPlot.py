...
def generate_default_data(workdir):
    ...
    生成默认数据集
    """
    data_path = os.path.join(workdir, 'input_files', 'demo_LinearRegressionPlot.csv')
...
def layout_interface(workdir=None):
    ...
    # 设置特殊参数
        ...
        symbol = st.selectbox('选择点的形状（有几组就选几组）：', symbol_list)
        sub_col1, sub_col2= st.columns(2) 
        with sub_col1:
            trendline_scope = st.radio('分组拟合 or 整体拟合:', ('trace', 'overall'), horizontal=True, index=0)
...
def Scatter_Plot(df_data, df_group=None, if_selected_color='离散颜色集', group_names=None, selected_text=None, trendline_scope='trace',
    ...
    if if_lg == 'yes':
        df_data = df_data.apply(lambda x: np.log10(x))
    # fig = px.scatter(
    #             df_data,
    #             x=df_data.columns[0],
    #             y=y_columns,
    ...
    #         )
    # 将数据转为长格式
    df_data = df_data.melt(id_vars=df_data.columns[0], var_name='Conditions', value_name='Values')
    # 将colors的颜色映射到Conditions
    color_map = {condition: color for condition, color in zip(df_data['Conditions'].unique(), colors)}
    # st.markdown(color_map)
    fig = px.scatter(df_data, x=df_data.columns[0], y='Values', 
                     color='Conditions', color_discrete_map=color_map, 
                     text='Conditions' if if_text == 'yes' else None,
                     opacity=opacity,
                     trendline='ols', trendline_scope=trendline_scope)
    # 设置所有点的固定大小为10
    fig.update_traces(marker=dict(size=marker_size, symbol=symbol if symbol else 'circle'))
    fig.update_layout(
        showlegend=if_legend, 
